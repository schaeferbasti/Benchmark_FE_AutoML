...
def main():
    amlb_task_ids = [
        # regression
        359944, 359929, 233212, 359937, 359950,
...
def run_and_save(feature_engineering_methods, task_id):
    ...
    df.to_csv('datasets/feature_engineered_datasets/' + name + '_original.csv', index=False)
...
def get_and_save_features(df_times, train_x, train_y, test_x, test_y, name, method, task_hint):
    ...
    if method == "autofeat":
        ...
        fe = limit(get_difer_features, wall_time=(4, "h"), memory=(32, "GB"))
        try:
            start_time = time.time()  #
            train_x, test_x = fe(train_x, train_y, test_x)
            end_time = time.time()  #
    ...
    elif method == "explorekit":
        fe = limit(get_explorekit_features, wall_time=(4, "h"), memory=(32, "GB"))
        try:
            start_time = time.time()  #
        ...
        fe = limit(get_fetch_features, wall_time=(4, "h"), memory=(32, "GB"))
        try:
            start_time = time.time()  #
            train_x, test_x = fe(train_x, train_y, test_x)
            end_time = time.time()  #
        ...
        fe = limit(get_mfe_features, wall_time=(4, "h"), memory=(32, "GB"))
        try:
            start_time = time.time()  #
            train_x, test_x = fe(train_x, train_y, test_x)
            end_time = time.time()  #
    ...
    df.to_csv('datasets/feature_engineered_datasets/' + name + '_' + method + '.csv', index=False)
