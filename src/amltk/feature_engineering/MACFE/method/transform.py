...
from src.amltk.feature_engineering.MACFE.method.metafeatures import get_metafeatures_dataset, get_histogram
from src.amltk.feature_engineering.MACFE.method.unary_transformations import Transformations as Unary_transformations
from src.amltk.feature_engineering.MACFE.method.binary_transformations import Transformations as Binary_transformations
...
def transform_unary(X, y, column_names, TRM_dataset):
    ...
    nbrs = NearestNeighbors(n_neighbors=1, algorithm='ball_tree', metric='euclidean', n_jobs=-1).fit(
        TRM_dataset[:, :-1])
