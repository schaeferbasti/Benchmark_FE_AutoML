...
def try_import_catboost():
    try:
        ...
        if sys.version_info >= (3, 11) and sys.platform == "darwin":
            error_msg += f"Detected your env as {sys.platform}. Please either downgrade your python version to below 3.11 or move to another platform. Then install via ``pip install autogluon.tabular[catboost]==1.1.2``"
        else:
            error_msg += f"A quick tip is to install via `pip install autogluon.tabular[catboost]==1.1.2`."
        raise ImportError()
...
def try_import_lightgbm():
    try:
        ...
        raise ImportError(
            ...
            f"A quick tip is to install via `pip install autogluon.tabular[lightgbm]==1.1.2`."
        )
    except OSError as e:
        raise ImportError(
...
def try_import_xgboost():
    try:
        ...
        raise ImportError(
            ...
            f"A quick tip is to install via `pip install autogluon.tabular[xgboost]==1.1.2`."
        )
...
def try_import_fastai():
    try:
        ...
        raise ImportError(
            f"Import fastai failed. A quick tip is to install via `pip install autogluon.tabular[fastai]==1.1.2`. "
        )
...
def try_import_autogluon_multimodal():
    try:
        ...
        raise ImportError(
            ...
            f"A quick tip is to install via `pip install autogluon.multimodal==1.1.2`.\n"
        )
