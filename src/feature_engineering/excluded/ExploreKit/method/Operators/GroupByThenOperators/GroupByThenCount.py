...
from src.feature_engineering.ExploreKit.method.Operators.GroupByThenOperators.GroupByThen import GroupByThen
...
class GroupByThenCount(GroupByThen):
    def __init__(self):
        super().__init__()
        self.missingValuesVal = 0
        self.countValuePerKey: Dict[Tuple, float] = {}
    ...
    def processTrainingSet(self, dataset: Dataset, sourceColumns: List[pd.Series], targetColumns: List[pd.Series]):
        ...
        self.missingValuesVal = np.mean(list(self.countValuePerKey.values()))
    ...
    def generate(self, dataset: Dataset, sourceColumns: List[pd.Series], targetColumns: List[pd.Series]) -> pd.Series:
        ...
        for i in range(dataset.getNumOfInstancesPerColumn()):
            ...
            if sourceValues not in self.countValuePerKey:
                column.append(self.missingValuesVal)
            else:
                column.append(self.countValuePerKey[sourceValues])
